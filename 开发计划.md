...
2. **用户认证**
   ...
   - ✅ JWT Token 管理
   - ✅ 用户信息存储
   - ✅ 401 错误自动处理（清除认证信息并跳转登录）
   - ✅ 租户信息管理工具

3. **服务层架构**
   - ✅ HTTP 请求封装（httpExecutor）
   - ✅ API 统一实例（api）
   - ✅ AuthService（登录、注册）
   - ✅ AccountService（用户信息、更新）
   - ✅ CompanyService（企业管理）
   - ✅ LogService（日志查询）
   - ✅ OssService（文件上传）
   - ✅ API 响应处理机制
...
### 2024-12-11
- ✅ 完成日志管理页面，使用新的 `findAll` 接口，支持分页和筛选
- ✅ 添加 401 错误处理，自动清除认证信息并跳转登录页
- ✅ 新增个人信息页面，支持修改用户名、手机号和头像上传
- ✅ 更新 WebLayout，用户名可点击跳转到个人信息页面
- ✅ 完善项目 README 文档
